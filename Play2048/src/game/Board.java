...
public class Board {
    ...
    public void addRandomTile() {
        // WRITE YOUR CODE HERE
        int ranBoardSpot = StdRandom.uniform(0, openSpaces.size());
        int tileValue = 0;
        double ranValue = StdRandom.uniform(0.0, 1.0);
        ...
        BoardSpot pos = openSpaces.get(ranBoardSpot);
        gameBoard[pos.getRow()][pos.getCol()] = tileValue;
    }
    ...
    public void transpose() {
        ...
        int[][] transGameBoard = new int[4][4];
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < gameBoard.length; j++) {
                transGameBoard[j][i] = gameBoard[i][j];
            }
        }

        for (int x = 0; x < gameBoard.length; x++) {
            for (int y = 0; y < gameBoard.length; y++) {
                gameBoard[x][y] = transGameBoard[x][y];
            }
        }
    ...
    public void flipRows() {
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            for (int j = 0; j < gameBoard.length / 2; j++) {
                ...
                gameBoard[i][j] = gameBoard[i][gameBoard[0].length - 1 - j];
                gameBoard[i][gameBoard[0].length - 1 - j] = temp;
            }
        }
    }
